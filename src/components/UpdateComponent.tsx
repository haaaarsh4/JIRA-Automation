...
const UpdateComponent = () => {
  ...
  const handleProcess = async (event: React.FormEvent) => {
    ...
    // Show loading toast
    const loadingToastId = toast.loading("Processing file and updating JIRA tickets...");

    try {
      const formData = new FormData();
      formData.append("file", file);
      formData.append("jiraField", selectedField);
      ...
      if (res.ok) {
        ...
        toast.success("JIRA Update Completed!", {
          description: `${data.updated} tickets updated successfully, ${data.failed} failed out of ${data.totalRows} total`,
          duration: 6000,
        });
        ...
        toast.error("Update Failed", {
          description: data.message || "An error occurred while updating JIRA tickets",
          duration: 5000,
        });
      }
    } catch (error) {
